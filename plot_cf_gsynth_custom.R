...
plot.gsynth <- function(
  ...
  if (legendOff == TRUE) {
    ...
    legend.pos <- "bottom" # edited (bottom)
  }
  ...
  if (type == "raw") {
    ...
    if (length(id) == 1 & !(id[1] %in% x$id.tr)) { ## error
      ...
      ## legend
      set.limits = c("time.bf")
      set.labels = c("Detected break (treatment) date")
      set.colors = c("#AAAAAA70")
      set.linetypes = c("solid")
      set.linewidth = c(2)
      ...
      p <- p + scale_colour_manual(limits = set.limits,
        ...
        guides(linetype = guide_legend(title=NULL, ncol=1),
               colour = guide_legend(title=NULL, ncol=1))
        ...
        if (length(id) == 1 | length(x$id.tr) == 1) { ## one treated unit
          ...
          if (raw == "none") { 
            ...
            p <- p + geom_line(aes(time, outcome,
                                   ...
                                   linetype = type)) 
            ## legend
            set.limits = c("tr","ct","time.bf")
            set.labels = c("Treated", "Estimated Y(0)", "Detected break (treatment) date")
            set.colors = c("black","steelblue", "#AAAAAA70")
            set.linetypes = c("solid","longdash", "solid")
            set.linewidth = rep(line.width[1],3)
            p <- p + scale_colour_manual(limits = set.limits,
                                         labels = set.labels,
                                         values =set.colors) +
              scale_linetype_manual(limits = set.limits,
